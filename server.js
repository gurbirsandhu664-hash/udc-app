...
function semanticMismatch(r,title){
  const t=norm(title), text=norm(`${r?.main_subject||''} ${r?.sub_subject||''} ${r?.breakdown||''} ${r?.explanation||''}`), n=String(r?.udc_number||'');
  const has=(re)=>re.test(t);
  const mentions=(re)=>re.test(text);
  // Strong title anchors: reject an AI answer that clearly jumps to an unrelated discipline.
  const anchors=[
    [/(education|teaching|pedagogy|schooling|teacher training)/,/(education|teaching|pedagogy|school|teacher|educational)/,/^37|^37[.:]/],
    [/(zoology|animal sciences?)/,/(zoology|animal|systematic zoology)/,/^59|^592\/599/],
    [/(agriculture|farming|crop|wheat|maize|corn|barley|harvest|harvesting)/,/(agriculture|crop|wheat|maize|corn|barley|harvest|farming)/,/^63|^633|^631/],
    [/(history|historical)/,/(history|historical)/,/^94/],
    [/(geography|geographical)/,/(geography|geographical)/,/^91/],
    [/(philosophy|metaphysics|logic|epistemology)/,/(philosophy|metaphysics|logic|epistemology|knowledge)/,/^(1|11|16|001)/],
    [/(language|linguistics|dictionary|lexicon|glossary)/,/(language|linguistics|literature|dictionary|lexicography)/,/^(80|81|82|821)/],
    [/(music|musical)/,/(music)/,/^78/],
    [/(art|arts|painting|sculpture|drawing|photography)/,/(art|painting|sculpture|drawing|photography)/,/^7/],
    [/(chemistry|chemical)/,/(chemistry|chemical)/,/^54|^66/],
    [/(physics|quantum|optics|mechanics|thermodynamics)/,/(physics|mechanics|optics|thermodynamics|quantum)/,/^53/],
    [/(biology|botany|zoology|ecology|genetics|microbiology)/,/(biology|botany|zoology|ecology|genetics|microbiology)/,/^57|^58|^59/]
  ];
  for(const [titleRe,textRe,numRe] of anchors){
    if(titleRe.test(t) && !(textRe.test(text) || numRe.test(n))) return `semantic anchor mismatch: ${n}`;
  }
  // A title containing science + technology + education should not be silently reduced to higher education.
  if(/education/.test(t)&&/science/.test(t)&&/technology/.test(t)&&!/(education|teaching|science|technology)/.test(text)) return 'education/science/technology mismatch';
  // 378 is higher education/universities; do not accept it for generic education unless the title says higher education/university.
  if(/^378/.test(n) && !/(higher education|university|universities|college|tertiary|postsecondary)/.test(t)) return '378 higher-education mismatch';
  return '';
}
function validate(r,title){
  const n=String(r?.udc_number||'').trim();
  if(!n||n==='0'||n==='—'||/^unknown|null|n\/a$/i.test(n))throw Error('Invalid UDC number');
  if(n.includes('004')&&!/(computer|computing|informatics|information technology|software|programming|data processing|artificial intelligence|machine learning|cyber|internet|database|ict)/i.test(title))throw Error('SEMANTIC_GUARD_004');
  const mismatch=semanticMismatch(r,title); if(mismatch) throw Error(mismatch);
  return {...r,title:r?.title||title,udc_number:n,sources:Array.isArray(r?.sources)?r.sources.filter(Boolean).slice(0,8):[],official_udc_match:!!r?.official_udc_match};
}
...
const exactTitleAliases=[
  [/^handbook of education science technology$/i,'handbook of education science and technology'],
  [/^education science and technology handbook$/i,'handbook of education science and technology'],
  [/^handbook education science and technology$/i,'handbook of education science and technology'],
];
...
function localClassify(title){let t=norm(title);for(const a of exactTitleAliases){if(a[0].test(t)){t=a[1];break}}for(const e of exact){if(e[0].test(t))return result(title,e[1],e[2],e[3],e[4],e[5],true)}
