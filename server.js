...
const MODELS = [process.env.GEMINI_MODEL,"gemini-3.8-flash","gemini-3.7-flash","gemini-3.6-flash","gemini-2.5-flash"].filter(Boolean).filter((v,i,a)=>a.indexOf(v)===i);
...
[/^music and entertainment$/i,"78","Music","Music","78 = Music. The title's dominant subject is music; entertainment is not used to force an additional symbol.","Official UDC Summary hierarchy match"],
...
[/^harvesting of wheat and maize$/i,"631.5:633.1","Agriculture","Agricultural operations relating to cereals","631.5 = Agricultural operations; 633.1 = Cereals/grain crops; : expresses a relation between the two subjects.","Reasoned from UDC Summary"],
];
...
app.get("/api/health",(_,res)=>res.json({ok:true,version:"V44 ULTRA",geminiConfigured:!!GEMINI_KEY,groqConfigured:!!GROQ_KEY,models:MODELS,authority:"UDC Summary",authorityUrl:SUMMARY_BASE}));
app.post("/api/classify",async(req,res)=>{const title=String(req.body?.title||"").trim();if(!title)return res.status(400).json({error:"Enter a book title."});
 ...
 const lc=localClassify(title); if(lc.official_udc_match || /^(history of india|geography of india|english drama|music and entertainment|science and technology|harvesting of wheat and maize)$/i.test(title.trim())) return res.json(lc);
 const errors=[];
 ...
 lc.provider_errors=errors.slice(-8);return res.json(lc);
});
app.listen(PORT,()=>console.log(`UDC V44 ULTRA on ${PORT}`));
