...
Treat the user's COMPLETE book title as a semantic description, not as a keyword-matching exercise. Identify the actual subject/topic, scope, process, object, discipline, geographic area, language, literary form, and document form expressed by the title. Generic words such as handbook, guide, introduction, study, principles, science, technology, methods, system, theory, problems, research, analysis, etc. must NOT determine the class by themselves.
Use the public UDC Summary as the primary abridged-style authority when available; when Google grounding is enabled, search UDC Summary/UDC Consortium and cross-check the exact class and notation before answering. Prefer the most specific UDC Summary class supported by the title.
...
004 is computer science/computing/data processing ONLY. Do not choose 004 because a title merely contains technology, technical, digital, system, application, method, science, or tool. Likewise, do not choose 37 education, 3 social sciences, 5 natural sciences, or any other broad class merely from a generic word.
Important semantic rule: classify the SUBJECT, not the grammatical wording. For example, a title containing a disease name belongs to the relevant medical/biomedical subject when supported by UDC; a title about a crop plus cultivation/harvesting belongs to agriculture; a title about a named philosophical topic belongs to philosophy; a literary title must be classified by language/literature/form when the title establishes those facets.
...
Never add an auxiliary just because it is possible. Every component and symbol must be justified by the title and the UDC schedule. Never invent a licensed MRF class. Never return DDC, 0, blank, null or N/A.
Before final JSON, internally do these checks: (1) state the title's actual subject in one phrase; (2) identify 2-3 plausible UDC candidates; (3) reject candidates caused only by generic title words; (4) verify the chosen notation against UDC Summary/UDC Consortium when grounding is available; (5) return the most specific supported candidate.
`;
...
function localClassify(title){const t=norm(title);for(const e of exact){if(e[0].test(t))return result(title,e[1],e[2],e[3],e[4],e[5],true)}
 ...
 if(/\bknowledge\b/.test(t)&&/\bmetaphysics\b/.test(t)&&/\blogic\b/.test(t))return result(title,"00+11+16","Generalities of knowledge / philosophy","Knowledge, metaphysics and logic","00 = Generalities of knowledge; 11 = metaphysics and ontology; 16 = logic and theory of knowledge; + = coordination.","UDC Summary-supported coordination",true);
 ...
 return result(title,"001","Knowledge, science and intellectual work","General knowledge / unresolved subject","001 = Knowledge, science and intellectual work. No narrower local UDC Summary concept was confidently established; an AI/official-reference lookup is required for a more specific classification.","Needs verification — no confident local subject match",false);
}
...
async function gemini(title,model,grounded){const body={contents:[{role:"user",parts:[{text:`${UDC_RULES}\nBOOK TITLE TO CLASSIFY: "${title}"\n\nReturn one final UDC classification. Do NOT classify from a single keyword. First determine what the book is actually about, then compare candidate UDC classes, and reject any broad class that is triggered only by generic wording. If the title contains several subjects, use UDC coordination/relation notation only when the title genuinely presents those subjects as co-equal or related. If a specific official UDC Summary class is found, use it rather than a broad parent class. If no exact public Summary class is available, use the narrowest defensible UDC hierarchy position and clearly state that it is a reasoned classification. Never output a made-up class. JSON only.`}]}],systemInstruction:{parts:[{text:UDC_RULES}]},generationConfig:{temperature:0,responseMimeType:"application/json",responseSchema:schema,maxOutputTokens:1800}};if(grounded)body.tools=[{googleSearch:{}}];const ac=new AbortController(),tm=setTimeout(()=>ac.abort(),28000);try{const u=`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`;const r=await fetch(u,{method:"POST",headers:{"Content-Type":"application/json","x-goog-api-key":GEMINI_KEY},body:JSON.stringify(body),signal:ac.signal});const txt=await r.text();let j;try{j=JSON.parse(txt)}catch{throw Error("Bad Gemini response")};if(!r.ok)throw Error(j?.error?.message||`Gemini HTTP ${r.status}`);const out=validate(parseJSON(j?.candidates?.[0]?.content?.parts?.map(x=>x.text||"").join("")),title);const chunks=j?.candidates?.[0]?.groundingMetadata?.groundingChunks||[];const gs=chunks.map(x=>x.web).filter(Boolean).map(x=>x.uri).filter(Boolean);if(!out.sources.length)out.sources=gs.slice(0,8);if(grounded && !gs.some(u=>/udcsummary\.info|udcc\.org/i.test(u)))throw Error("NO_OFFICIAL_UDC_SUMMARY_EVIDENCE");
...
app.post("/api/classify",async(req,res)=>{const title=String(req.body?.title||"").trim();if(!title)return res.status(400).json({error:"Enter a book title."});
 ...
 const errors=[];
 // Unknown titles MUST go through semantic classification before the local broad fallback.
 // Grounded Gemini is tried first so the model can search UDC Summary instead of guessing from keywords.
 if(GEMINI_KEY){for(const model of MODELS){try{return res.json(await gemini(title,model,true))}catch(e){errors.push(`${model}/search: ${e.message}`)}}}
 if(GEMINI_KEY){for(const model of MODELS){try{return res.json(await gemini(title,model,false))}catch(e){errors.push(`${model}/plain: ${e.message}`)}}}
